...
AULC_tuk %>% filter(grepl('control', comparison)) %>% 
  mutate(comparison=factor(comparison, levels = c('RPS-control', 
                                                  'Acid-control', 
                                                  'RCS-control'))) %>% 
  ggplot(aes(x=comparison, y=estimate, ymin=conf.low, ymax=conf.high,color=comparison)) +
  geom_hline(yintercept = 0) +
  geom_pointrange(size=1.52) +
  geom_text(aes(x=comparison,y=10,
                label=paste('P=', round(adj.p.value, 2)))) + ggtitle('Estimates and 95% confidence intervals')
